...
```{r testing data frame diff error from shiny}
...
test_2 <- test %>% 
  mutate(graphs = pmap(.l = list(stress_tibble_color$colors, stress_tibble_color$stress,
                     stress_tibble_color$name), .f = my_plot_function_v2))


graphs <-  pmap(.l = list(stress_tibble_color$colors, stress_tibble_color$stress,
                     stress_tibble_color$name), .f = my_plot_function_v2)
...
my_plot_function_v2 <- function(stress_tibble_color$colors, stress_tibble_color$stress,
                     stress_tibble_color$name){
  ggplot(data = stress_tibble_color) +
  geom_point(aes(x = 10, y = 10), size = 100, shape = 21, fill = stress_tibble_color$colors) +
  geom_point(aes(x = 8.5, y = 15), size = 10, shape = 21, fill = "white") +
  geom_point(aes(x = 11.5, y = 15), size = 10, shape = 21, fill = "white") +
  geom_curve(aes(x = 8.5, y = 8, xend = 11.5, yend = 8), curvature = curve_function(stress_tibble_color$stress), size = 5,
             color = "white", lineend = "round") +
  geom_label(aes(x = 10, y = 19.5, label = stress_tibble_color$name), size = 8, color = "blue",
             label.padding = unit(0.5, "lines")) +
    xlim(c(0,20)) + ylim(c(0,20)) +
    theme_void()
}
...
test <- stress_tibble_color %>% 
  mutate(graphs = pmap(.l = list(.$colors, .$stress,
                     .$name), .f = my_plot_function_v2))
